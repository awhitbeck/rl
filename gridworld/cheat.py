...
class Game:
    ...
    def reset(self):
        ...
        self.xpos = self.rng.integers(low=int((self.imagex-1)/2)-10,high=int((self.imagex-1)/2)+10)
        self.ypos = self.rng.integers(low=int((self.imagex-1)/2)-10,high=int((self.imagex-1)/2)+10)
        self.generate_scene()

        return np.array([self.xpos,self.ypos])#self.image
    ...
    def step(self,action):
        ...
        #return self.image,self.reward(),self.done()
        return np.array([self.xpos,self.ypos]),self.reward(),self.done()
...
num_episodes=100
batch_size=64
output_dir='model_output/fourth_agent'
